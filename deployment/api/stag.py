...
# Remote destination
remote_destination = "/data/api/stag"
...
with get_connection() as conn:
    ...
    run_command_in_venv(
        conn,
        remote_destination,
        "SQLALCHEMY_DATABASE_URI=postgresql://pgusername:pg-password-to-update@127.0.0.1/pgusername_ads_stag flask db upgrade",
        remote_destination,
    )
    run_remote(
        conn,
        f"cp {remote_destination}/deployment/api/staging.systemctl.service /etc/systemd/system/api_stag.service",
        use_sudo=True,
    )
    run_remote(conn, "systemctl enable api_stag", use_sudo=True)
    run_remote(conn, "systemctl daemon-reload", use_sudo=True)
    run_remote(conn, "systemctl restart api_stag", use_sudo=True)
    # Check logs on remote machine using: journalctl -xeu api_stag.service
